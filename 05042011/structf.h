...
// Structures are passed to a function using a pointer
...
void readFile(FILE *fp, struct node *currentNode) {
	...
	while(fgets(line,MAXLINE,fp)!=NULL) {
		...
		setNull(son);//   <--------------------------------------------added this line here (risto)
		...
		currentNode=son;
		fillRegisterArrays(currentNode);//   <-----------------------------------------here I call fill register arrays (risto)
...
/*void readFile(FILE *fp, struct node *currentNode) {
	...
	char line[MAXLINE];
	struct node *son=(struct node *)malloc(sizeof(struct node));	// Allocate memory for a new node
	//setNull(son);
	son->prev=currentNode;	// Tell to the next structure that its previous one is the current node
    if(fgets(line,MAXLINE,fp)!=NULL) {		// Take a line in order to perform some actions on it
        cleanString(line);	// Call the cleanString function to eliminate all characters that are not needed
		currentNode->instruction=recognizeCommand(line); // Store the instruction of the actual line and recognize the instruction
		...
		readFile(fp,son);		// Read the next line
    }
	else {
		currentNode->next=NULL;	// Don't create the new structure (node) because the file ended
		currentNode->future1=NULL;
		currentNode->future2=NULL;
	}
}
...
void fillRegisterArrays(struct node *currentNode){
	...
	for(i = 46; i < 50; i++){
		if(recognizeCommand(currentNode->line) == i){
		currentNode->accessed[30] = 1; //lo was set
		currentNode->accessed[31] = 1;//hi was set
		}
		if(register_counter == 1){
			currentNode->valueSet[position] = 1;
			currentNode->accessed[position] = 0;
...
int recognizeCommand(char instr[])
{
	...
	/* two register branches */
	if(checkPrefix(instr,"beq",3) || checkPrefix(instr,"bgt",3) || checkPrefix(instr,"blt",3))		return 25;
	if(checkPrefix(instr,"ble",3) || checkPrefix(instr,"bge",3) || checkPrefix(instr,"bne",3))		return 25;
