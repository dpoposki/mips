...
void openFile(struct treeNode *formulaToCheck[], int *numFormula, char formulaPath[]){
    ...
    if(!fp) MessageBox(NULL, L"FORMULA NOT FOUND", L"test", MB_OK);
	else { 
...
void manualFormula(struct treeNode *formulaToCheck[], int *numFormula, char formulaPath[]){
	char formula[MAXFORMULA];
	FILE *fp;
    fp=fopen(formulaPath, "r");
...
string printFormula(struct treeNode *currentTreeNode){
