...
void readFormula(struct treeNode *currentTreeNode, char formula[], int *index, int level){
	// Binary: A = implication, B = conjunction, C = disjunction, D = negation
	// Unary: E = AF, F = AG, G = EF, H = EG, I = AX, J = EX
	// Binary; K = AU, L = EU
...
void printFormula(struct treeNode *currentTreeNode){
    if (currentTreeNode->element=='A'){
        ...
        printf(" -> ");
		printf("[%d]",currentTreeNode->level);
		...
		printf("%c",currentTreeNode->element);
		printf("[%d]",currentTreeNode->level);
	}
}
